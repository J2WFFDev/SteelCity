...
class MinimalBridge:
    ...
    async def connect_bt50_sensor(self):
        ...
        try:
            # Try direct connection first (proven to work)
            self.log("info", "Using direct connection method for BT50")
            client = BleakClient(self.bt50_sensor_mac, device="hci0")
            ...
            self.log("error", "BT50 direct connection failed, trying discovery", {
                "error": str(e),
                "type": type(e).__name__
            })
            ...
            try:
                ...
                bt50_device = await BleakScanner.find_device_by_address(self.bt50_sensor_mac, timeout=10.0)
                if bt50_device:
                    self.log("info", "BT50 found via discovery, connecting", {
                        "name": bt50_device.name,
                        "rssi": getattr(bt50_device, 'rssi', 'unknown')
                    })
                    
                    client = BleakClient(bt50_device, device="hci0")
                    await client.connect(timeout=20.0)
                    
                    self.log("success", "BT50 sensor connected via discovery", {
                        "connected": client.is_connected,
                        "services": len(list(client.services)) if client.services else 0
                    })
                    ...
                    self.log("error", "BT50 sensor not found in discovery")
                    return None
                    
            except Exception as e2:
                self.log("error", "BT50 sensor connection completely failed", {
                    "discovery_error": str(e2),
