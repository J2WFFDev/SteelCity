...
class Commander:
    def __init__(self, cfg: CommanderConfig):
        ...
        self._last_tail_flag: Optional[int] = None
    ...
    def _on_control(self, payload: bytes):
        ...
        if payload and len(payload) == 14 and payload[0] == 0x01:
            ...
            self._last_tail_flag = tail_flag

            if (prev != 0x01 and tail_flag == 0x01 and 0 < secs <= self._max_secs
                    and all(b == 0x00 for b in mid)):
                self._apply_cmd("START", float(secs))
                return

            if prev != 0x02 and tail_flag == 0x02:
        ...
        if getattr(self.cfg, "debug_hex", False):
            prev_hex = payload.hex()
            if len(prev_hex) > 64:
                prev_hex = prev_hex[:64] + "…"
            print(f"\n[ctl] Ignored payload: len={len(payload)} hex={prev_hex}")
